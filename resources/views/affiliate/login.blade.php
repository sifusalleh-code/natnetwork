@extends('layouts.client', ['title' => 'Log masuk Affiliate | NatNetwork Synergy'])

@section('content')
<section class="auth-card">
    <p class="eyebrow">Portal Affiliate</p>
    ...
    <form method="post" action="{{ route('affiliate.otp.send') }}" class="form-stack">
        @csrf
    ...
    @if ($otpEmail)
        <form method="post" action="{{ route('affiliate.otp.verify') }}" class="form-stack top-gap">
            @csrf
    ...
    <p class="top-gap">Belum berdaftar? <a href="{{ route('affiliate.register') }}">Daftar sebagai affiliate</a></p>
</section>
@endsection
